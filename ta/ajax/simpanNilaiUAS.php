...
INNER JOIN ubaya.Mahasiswa mhs ON mk.NRP=mhs.NRP WHERE mk.KodeMkBuka = '$idKls[0]' AND mk.KP = '$idKls[1]' ORDER BY mk.NRP";
$querySelect = mysql_query("SELECT id_kt FROM tgsakhir.jadwalkuistugas as jkt WHERE jkt.id_kelas='$gabungan' AND jkt.tipe='UAS' ORDER BY jkt.jenis_kt,jkt.ke ASC");
echo "SELECT id_kt FROM tgsakhir.jadwalkuistugas as jkt WHERE jkt.id_kelas='$gabungan' AND jkt.tipe='UAS' ORDER BY jkt.jenis_kt,jkt.ke ASC";
$b=0;
...
for($i=0;$i<$jumRowUAS;$i++)
{
	$fetchSelect = mysql_fetch_array($querySelect);
	$UAS[$i]=$fetchSelect[id_kt];
}
	...
	//$queryUpdate = mysql_query("UPDATE jadwalkuistugas SET persen=$a WHERE id_kt='$fetchSelect[id_kt]'");
		...
		$a = $_POST[nilaiUAS][$i][$j];
	mysql_query("UPDATE tgsakhir.nilai as nli SET nli.nilai=$a WHERE nli.id_kt='$UAS[$j]' AND nli.id_mhs=$murid[$i]");
		$b++;
		echo "UPDATE tgsakhir.nilai as nli SET nli.nilai=$a WHERE nli.id_kt='$UAS[$j]' AND nli.id_mhs=$murid[$i]";
		}
